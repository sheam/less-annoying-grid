...
) =>
{
    ...
    const setPaginationDataSafe = (newCurrentPage : number, pageSize : number) =>
        setPagination({
            currentPage: clamp(
                newCurrentPage,
                1,
                getTotalPages(props.totalCount, pageSize)
            ),
            pageSize,
        });
    ...
    return (
        <tfoot
            title={
                preventPaging
                    ? 'paging disabled when the grid contains unsaved data'
                    : ''
            }
        >
            <tr>
...
) : JSX.Element
{
    ...
    numJumpButtons = Math.min(
        numJumpButtons || defaultNumJumpButtons,
        totalPages
    );
        ...
        <span className="bn-jumps">
            {pageJumps.map(n => (
                <button
                    ...
                    onClick={e =>
                        setPage(parseInt((e.target as any).value, 10))
                    }
                >
                    {n}
                </button>
...
) : number[]
{
    ...
    pageSizes = pageSizes
        .slice(0, 1)
        .concat(pageSizes.slice(1).filter(n => n <= totalItemCount));
